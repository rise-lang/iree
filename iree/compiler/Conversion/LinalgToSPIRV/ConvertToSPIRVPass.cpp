...
/// Convert subgroup level vector transfert+contract to SPIR-V cooperative
/// matrix if those are supported.
...
class TransferToCoopMatLoadStore final : public SPIRVOpLowering<OpTy> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    // TODO(thomasraoux): support transpose.
...
class VectorContractToCoopMatmul final
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    auto loadA = operands[0];
    auto loadB = operands[1];
    auto loadC = operands[2];
    ...
    if (!isParallelIterator(iteratorTypes[0]) ||
      ...
      return failure();
    // cooperative matrix only support row major muladd. To handle column major
    // matrix multiplication we can load/store matrix with coolumn major flag to
    // transpose the data.
...
void ConvertToSPIRVPass::runOnOperation() {
  ...
  patterns.insert<HALInterfaceLoadConstantConverter, IREEPlaceholderConverter,
                  LinalgReshapeConverter,
                  TransferToCoopMatLoadStore<vector::TransferReadOp>,
